...
export function Filters({ currentSort }: { currentSort: string | null }) {
  ...
  const mainCrop = params.get("mainCrop") ?? "";
  ...
  function pushWith(updates: Record<string, string | null>) {
    ...
    next.delete("page");
    startTransition(() => router.push(`/consultations?${next.toString()}`));
  }
  ...
  const hasFilters = Boolean(search || status || mainCrop);
  ...
  return (
    ...
    >
      ...
      <select
        value={mainCrop}
        onChange={(event) => pushWith({ mainCrop: event.currentTarget.value || null })}
      ...
      {hasFilters ? (
        <button
          type="button"
          onClick={() => pushWith({ search: null, status: null, mainCrop: null })}
