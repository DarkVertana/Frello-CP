...
export function Filters({ currentSort }: { currentSort: string | null }) {
  ...
  const paymentKind = params.get("paymentKind") ?? "";
  ...
  function pushWith(updates: Record<string, string | null>) {
    ...
    next.delete("page");
    startTransition(() => router.push(`/orders?${next.toString()}`));
  }
  ...
  const hasFilters = Boolean(search || status || paymentKind || from || to);
  ...
  return (
    ...
    >
      ...
      <select
        value={paymentKind}
        onChange={(event) =>
          pushWith({ paymentKind: event.currentTarget.value || null })
        }
        aria-label="Filter by payment kind"
        className="h-10 rounded-input border border-border bg-card px-3 text-sm text-foreground"
      >
